...
func spawn_random(z):
	var rng_spawn = rng.randf_range(1, foreWaveSpawnRange + 1)
	rng_spawn = int(rng_spawn)
	
	for n in rng_spawn:
		var rand = rng.randf_range(1, 6)
		rand = int(rand)
		print(rand)
		match rand:
			1: spawn1(z)
